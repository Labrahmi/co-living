...
function Hero() {
    ...
    const [loadSVG_1, setloadSVG_1] = useState(false);

    // use effect

    // useEffect with an empty dependency array to run only once after component mount
    useEffect(() => {
    ...
    const heroIcon = useRef<HTMLDivElement>(null);

    return (
        <div className='flex max-w-full items-center flex-col lg:flex-row gap-4 bg-white relative lg:p-12 p-2 py-6 border lg:rounded-xl rounded-md lg:mt-28 xl:mt-44'>
            <div className='card-one lg:w-1/6 flex flex-col gap-2'>
                <div className='w-full flex justify-start relative'>
                    <div className='absolute left-0 top-0 w-full h-full flex justify-center items-center'>
                        {loadSVG_1 ? <div className='' ref={heroIcon}><myIcons._1 /></div> :
                            <div className='w-full flex flex-col space-y-2'>
                                <div className='bg-zinc-200 w-[100%] flex rounded-2xl text-zinc-200 animate-pulse'>.</div>
                                <div className='bg-zinc-200 w-[75%] flex rounded-2xl text-zinc-200 animate-pulse'>.</div>
                                <div className='bg-zinc-200 w-[50%] flex rounded-2xl text-zinc-200 animate-pulse'>.</div>
                            </div>
                        }
                    </div>
            ...
            <div className='card-one px-4 flex flex-col gap-2 text-center'>
                ...
                <div className='flex xl:flex-row flex-col gap-2 font-medium'>
                    <button className='border hover:border-red-100 p-2 rounded text-nowrap text-sm flex justify-center items-center gap-1 hover:text-red-500 group'>
                        <div>Continue with Google</div>
                        <Icons.GoogleIcon className='w-4 h-4 group-hover:text-red-500' />
                    </button>
                    <button className='border hover:border-blue-100 p-2 rounded text-nowrap text-sm flex justify-center items-center gap-1 hover:text-blue-500 group'>
